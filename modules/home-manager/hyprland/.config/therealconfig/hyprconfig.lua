...
hl.config({
  general = {
    gaps_in = 1,
    gaps_out = 0, -- could also use a table { top=0, left=0, right=0, bottom=0  },

    border_size = 2,

    col = {
      active_border = { colors = { "rgba(8a9aaaee)", "rgba(9aabbbee)" }, angle = 45 },
...
hl.bind(
  mainMod .. " + SHIFT + V",
  hl.dsp.exec_cmd(
    "echo -e 'Yes\\nNo' | rofi -dmenu -p 'Clear clipboard history?' | grep -q 'Yes' && cliphist wipe && notify-send 'Clipboard' 'History cleared!'"
  )
)
